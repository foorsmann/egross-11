...
(function(){
  ...
  function qgSyncSliderQtyUI(qtyEl, sendQty) {
    ...
    var isLow = (max > 0 && max < step);

    // low-stock styling aliniat cu tema
    qtyEl.classList.toggle('text-red-600', isLow);
    if (isLow) { qtyEl.style.color = '#e3342f'; } else { qtyEl.style.color = ''; }
    ...
    if (dbl) {
      var disabled = isLow; // cand stoc < min_qty, dezactivat
